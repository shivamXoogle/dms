...
async function fetchProducts() {
    try {
        ...
        const URL = base_url + 'Welcome/GetProductsDataWithStock/' + pagination.activeIndex + '/5/' + searchKey;
        ...
        if (response.status == 200) {
            ...
            for (value of response.data) {
                let { ProductID,ProductImage, ProductName, Price, Size ,Color,createdAt,quantity} = value;
                tableData += "<tr>" +
                    "<td>" + ProductID + "</td>" +
                    "<td>" + ProductName + "</td>" +
